    ...
    return (
        <section className='py-5 border-bottom' id='sectionExperience'>
            <div className='container px-xl-5 py-xl-3 p-sm-0'>
                ...
                <div className={`accordion accordion-flush border-top ${theme === 'dark' ? 'bg-dark' : 'bg-light'}`} id='accordionPanelsStayOpenExample'>
                    {Experiences.map((experience,index) => 
                        <div key={experience.id} className='accordion-item'>
                            <h2 className='accordion-header'>
                                <button className={`accordion-button ${theme === 'dark' ? 'bg-dark text-light' : 'bg-light text-dark'}`} type='button' data-bs-toggle='collapse' data-bs-target={`#panelsStayOpen-collapse${index}`} aria-expanded='true' aria-controls={`panelsStayOpen-collapse${index}`}>
